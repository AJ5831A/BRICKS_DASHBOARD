...
const createClass = async(req , res)=>{
    try{
        ...
        notificationService.sendClassScheduledNotification(
            batch,
            newClass,
            `${instructor.fullname.firstname} ${instructor.fullname.lastname || ''}`.trim()
...
const updateClass = async (req , res) =>{
    try{
        ...
        if(updatedFields.length>0){
            notificationService.sendClassUpdatedNotification(
                classData.batch,
                classData,
                classData.instructorName,
                updatedFields
            ).catch(err=>console.error('Error sending class updated notification:', err));
        }
...
const deleteClass = async (req , res)=>{
    try{
        ...
        notificationService.sendClassCancelledNotification(
            classData.batch,
            classData,
            classData.instructorName,
            "Class has been cancelled by the instructor"
        ).catch(err=>console.error('Error sending class cancelled notification:', err));
